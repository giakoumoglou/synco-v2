# --------------------------------------------------------
# Reversible Vision Transformer (RevViT) implementation
...
class RevViT(nn.Module):
    def __init__(
        ...
        self.pos_embeddings = nn.Parameter(torch.zeros(1, num_patches, self.embed_dim))
        ...
        self.head = (
            nn.Linear(2 * self.embed_dim, num_classes, bias=True)
            if num_classes > 0
            else nn.Identity()
        )
...
def rev_vit_small_patch16_224(pretrained=False, **kwargs):
    pass


def rev_vit_base_patch16_224(pretrained=False, **kwargs):
    pass


def rev_vit_large_patch16_224(pretrained=False, **kwargs):
    pass
