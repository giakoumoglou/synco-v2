...
from src.utils import load_pretrained, load_checkpoint, save_checkpoint, get_grad_norm, auto_resume_helper, reduce_tensor
...
def main(config):    
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        if dist.get_rank() == 0 and (epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1)):
            save_epoch = (epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1))
            save_checkpoint(config, epoch, model_without_ddp, max_accuracy, optimizer, lr_scheduler, logger, save_epoch=save_epoch)
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler):
    """
    One epoch training for linear evaluation.
    ...
    args:
        ...
        epoch (int): epoch
        mixup_fn (function): mixup function
...
def validate(config, data_loader, model):
    ...
    args:
        ...
        model (nn.Module): model
    returns:
        acc1 (float): top-1 accuracy
        acc5 (float): top-5 accuracy
        loss (float): loss
    """
    criterion = torch.nn.CrossEntropyLoss()
    model.eval()
