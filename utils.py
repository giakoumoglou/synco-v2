...
def save_checkpoint(config, epoch, model, max_accuracy, optimizer, lr_scheduler, logger, save_epoch=False):
    ...
    latest_path = os.path.join(config.OUTPUT, 'checkpoint.pth')
    torch.save(save_state, latest_path)

    if save_epoch:
        epoch_path = os.path.join(config.OUTPUT, f'ckpt_epoch_{epoch}.pth')
        logger.info(f"==============> Saving epoch checkpoint to {epoch_path}")
